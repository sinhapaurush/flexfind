...
class Search
{
    ...
    private $search_result = "p.id, p.title, p.description, p.url, p.content, d.name, d.favicon FROM page p INNER JOIN domain d ON p.domain = d.id";
    ...
    private function filterSortAndLimit()
    {
        ...
        if ($alreadyShown === "IN ()") {
            ...
            $string = "AND p.id NOT {$alreadyShown} {$this->sortByScoreAndAuthority} LIMIT 0, 10";
    ...
    public function searchInLink()
    {
        $filterSortAndLimit = $this->filterSortAndLimit();
        $query = mysqli_query(
            $this->con,
            "SELECT {$this->search_result} WHERE url LIKE '%{$this->keyword}%' {$filterSortAndLimit};"
        );
    ...
    private $searchable_columns = ['title', 'description', 'keywords', 'heading', 'content', 'hlevels', 'url'];
    ...
    public function weakFindInAnyCol()
    {
        ...
        $query = mysqli_query($this->con, "
            SELECT {$this->search_result} WHERE {$condition} {$filterSortAndLimit}; 
        ");
        // return "SELECT {$this->search_result} WHERE {$condition} {$filterSortAndLimit}; ";
        return $this->indexToAlreadyShownAndReturn($query);
